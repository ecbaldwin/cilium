...
func (r *NeighborReconciler) Reconcile(ctx context.Context, p ReconcileParams) error {
	...
	for i, n := range newNeigh {
		...
		// An unnumbered peer has no PeerAddress, and gobgp sources the session
		// from the peering interface's own link-local. A localAddress override
		// (from CiliumBGPNodeConfigOverride) would replace that derivation with
		// an address gobgp cannot reach a link-local peer from, so ignore it.
		if n.PeerAddress == nil && ptr.Deref(peer.LocalAddress, "") != "" {
